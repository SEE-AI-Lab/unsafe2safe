...
The example config is [`config.example.yaml`](config.example.yaml). It reproduces the paper-era FlowEdit SD3 route: 50 steps, `n_avg=1`, source guidance 3.5, target guidance 13.5, `n_min=0`, `n_max=33`, seed 42, and a maximum resolution of 1536 pixels. The command takes one source text column and one target text column. Rows whose `file` value begins with `val` are excluded by default, matching the paper's train-only generation convention.

The CSV must contain the configured file, source, and target columns. File values are relative paths; `image-root/file` is read and the generated image is written to `output-dir/file`. Existing outputs are skipped unless `--overwrite` is supplied.
