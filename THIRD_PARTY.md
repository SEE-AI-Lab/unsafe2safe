...
`unsafe2safe/external.py`, `unsafe2safe/attention.py`, and
`unsafe2safe/model.py` are the Unsafe2Safe-specific import and model
...
- Install: clone the repository, check out one selected revision, install its
  ...
  [`unsafe2safe/adapters/ominicontrol/README.md`](unsafe2safe/adapters/ominicontrol/README.md).
- License: upstream [Apache-2.0 LICENSE](https://github.com/Yuanshi9815/OminiControl/blob/main/LICENSE).

`unsafe2safe/adapters/ominicontrol/` contains only the Unsafe2Safe dataset mapping and
...
- Install: clone the repository, check out that revision, install the
  ...
  described in [`unsafe2safe/adapters/flowedit/README.md`](unsafe2safe/adapters/flowedit/README.md).
- License: upstream [MIT LICENSE](https://github.com/fallenshock/FlowEdit/blob/main/LICENSE).

`unsafe2safe/adapters/flowedit/` contains only the Unsafe2Safe CSV condition mapping and
...
- Install: clone that revision separately and install it in the dedicated
  BLIP-2 environment described in [`unsafe2safe/README.md`](unsafe2safe/README.md).
...
`unsafe2safe/adapters/lavis/blip2_captioning.py`; its hard-coded dataset paths, GPU/cache
...
[`unsafe2safe/adapters/image_mae/dataset.py`](unsafe2safe/adapters/image_mae/dataset.py). It is
