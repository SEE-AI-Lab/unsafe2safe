...
`unsafe2safe/instruct_pix2pix.py`, `unsafe2safe/safe_attention.py`, and
`unsafe2safe/unsafe2safe_model.py` are the Unsafe2Safe-specific import and model
...
- Install: clone the repository, check out one selected revision, install its
  ...
  [`unsafe2safe/ominicontrol/README.md`](unsafe2safe/ominicontrol/README.md).
- License: upstream [Apache-2.0 LICENSE](https://github.com/Yuanshi9815/OminiControl/blob/main/LICENSE).

`unsafe2safe/ominicontrol/` contains only the Unsafe2Safe dataset mapping and
...
- Install: clone the repository, check out that revision, install the
  ...
  described in [`unsafe2safe/flowedit/README.md`](unsafe2safe/flowedit/README.md).
- License: upstream [MIT LICENSE](https://github.com/fallenshock/FlowEdit/blob/main/LICENSE).

`unsafe2safe/flowedit/` contains only the Unsafe2Safe CSV condition mapping and
...
`unsafe2safe/blip2_captioning.py`; its hard-coded dataset paths, GPU/cache
...
[`unsafe2safe/image_mae_dataset.py`](unsafe2safe/image_mae_dataset.py). It is
