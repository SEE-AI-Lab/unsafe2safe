...
def run_internvl_pair_batch(
    ...
    *,
    ...
    for left_path, right_path in zip(left_image_paths, right_image_paths):
        left_img = preprocess_image(left_path, image_size=image_size)
        right_img = preprocess_image(right_path, image_size=image_size)
        pixel_values_list.extend([left_img, right_img])
        num_patches_list.append(2)
    pixel_values = torch.stack(pixel_values_list).to(device=device, dtype=torch.bfloat16)
    questions = []
