...
def get_parser(**parser_kwargs):
    ...
    "--csv_path",
    type=str,
    default="triplets.csv",
    help="Path to the CSV file containing image paths and captions"
    )
...
class SetupCallback(Callback):
    ...
    def on_keyboard_interrupt(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            ckpt_path = os.path.join(self.ckptdir, "last-v2.ckpt")
...
def all_gather(data):
    ...
    if not isinstance(data, torch.Tensor):
        ...
        tensor = torch.tensor(list(buffer), dtype=torch.uint8, device="cuda")
    else:
        origin_size = data.size()
        tensor = data.reshape(-1).to("cuda")
...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
        ...
        self._tensorboard = self._testtube
    ...
    def log_local(self, save_dir, split, images, prompts,
        ...
        names = {
            "input_private": "private-input",
            "target_public": "public-target",
            "reconstruction": "recon",
            "samples_edited": "edited-output",
            "samples_quantized": "edited-quantized",
            "diffusion_row": "diffusion-progress",
            "progressive_row": "progressive",
        }
        # print(root)
        for k in images:
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            prompts = batch.get("caption_edit", [""] * self.max_images)[:self.max_images]
...
class CUDACallback(Callback):
    def on_train_epoch_start(self, trainer, pl_module):
        # Reset CUDA peak memory stats at the start of epoch
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        device = trainer.strategy.root_device
        torch.cuda.synchronize(device)
        max_memory = torch.cuda.max_memory_allocated(device) / 2**20  # Convert to MiB
        epoch_time = time.time() - self.start_time

        try:
            strategy = trainer.strategy
            max_memory = strategy.reduce(max_memory)
            epoch_time = strategy.reduce(epoch_time)
...
if __name__ == "__main__":
    ...
    ckpt = os.path.join(logdir, "checkpoints", "last.ckpt")#, "epoch=000071.ckpt")
    resume = False
