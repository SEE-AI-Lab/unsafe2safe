...
implementation. The active paper path is the small set of modules at the
package root: `data.py`, `model.py`, `attention.py`, and `external.py`.

The legacy batch editor is exposed through
`unsafe2safe/scripts/run_unsafe2safe.sh` for compatibility with existing
