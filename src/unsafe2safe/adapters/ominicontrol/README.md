...
paper. Paths are relative to `unsafe2safe/adapters/ominicontrol/config.yaml` unless absolute
...
Update the data paths in `config.yaml` before training. Do not commit private
...
```bash
export PYTHONPATH="$PWD:$OMINICONTROL_ROOT:${PYTHONPATH:-}"
python -m unsafe2safe.adapters.ominicontrol.generate_unsafe2safe \
